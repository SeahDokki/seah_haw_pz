...
local handlers = {}   -- every registered handler, in registration order
local slowHandlers = {}
...
function SHAW.Tick.registerSlow(spec)
    ...
    table.insert(slowHandlers, spec)
end
...
local function sessionFor(player)
    ...
    if not session or session.player ~= player then
        session = {
            player = player,
            fast = resolve(player, handlers),
            slow = resolve(player, slowHandlers),
            lastRun = {},
        }
        sessions[id] = session
...
--- Drop the cached handler list so the next tick rebuilds it. Call after
--- anything that can change which traits apply - the debug menu does.
function SHAW.Tick.invalidate()
    sessions = {}
    SHAW.log("handler cache cleared")
end
