...
# ------------------------------------------------------- sandbox coverage --
options = io.open("SHAW/42/media/sandbox-options.txt", encoding="utf-8").read()
...
GETTEXT = re.compile(r'getText\(\s*"(IGUI_SHAW_\w+)"')
LABELFIELD = re.compile(r'label\s*=\s*"(IGUI_SHAW_\w+)"')

used = set()
for lua in glob.glob("SHAW/42/media/lua/**/*.lua", recursive=True):
...
if missing:
    ...
    print("getText  all %d IGUI_SHAW keys used in Lua are defined" % len(used))
